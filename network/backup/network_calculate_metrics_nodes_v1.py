...
def calculate_all_metrics(df_group, month_start):
    """Calculate both network-wide and politician-specific metrics."""
    try:
        ...
        if len(Graph) > 0:
            ...
            logger.info("calculating: closeness_centrality")
            ...
            # )
            # logger.info("calculating: katz")
            # katz = nx.katz_centrality_numpy(Graph, weight="weight", max_iter=100)
            # katz = nx.katz_centrality(Graph, weight="weight", max_iter=100)
        ...
        return pd.Series(network_metrics), pd.DataFrame(politician_metrics)
...
def process_all_data():
    ...
    try:
        ...
        for i, month_start in enumerate(months, 1):
            ...
            network_metrics, politician_metrics = calculate_all_metrics(
                df_month, month_start
            )

            if not network_metrics.empty:
                network_metrics_list.append(network_metrics)
            if not politician_metrics.empty:
                all_politician_metrics.append(politician_metrics)

            logger.info(f"Completed processing for {month_start}")

        # Combine results
        network_df = pd.DataFrame(network_metrics_list)
        politician_df = pd.concat(all_politician_metrics, ignore_index=True)

        # Calculate party aggregations
        party_df = (
            politician_df.groupby(["month_start", "party"])
            .agg(
                {
                    "pagerank": ["mean", "std"],
                    "in_degree": ["mean", "std"],
        ...
        politician_df.to_csv(
            os.path.join(OUTPUT_DIR, f"politician_metrics_{timestamp}.csv"), index=False
        )
        party_df.to_csv(
        ...
        logger.info(f"Saved politician metrics to: politician_metrics_{timestamp}.csv")
        logger.info(f"Saved party metrics to: party_metrics_{timestamp}.csv")

        return network_df, politician_df, party_df
...
if __name__ == "__main__":
    try:
        network_metrics, politician_metrics, party_metrics = process_all_data()
