...
class OptimizedPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            max_workers: Number of parallel workers for inference
            pipeline_batch_size: Batch size for transformer pipelines
        ...
        self.source_table = "full_interaction_chain"
        ...
        # Semaphore to control concurrent batch processing
    ...
    async def process_batch(self, batch: List[Dict]) -> None:
        ...
        async with self.batch_semaphore:
            ...
            try:
                ...
                logger.info(
                    f"Batch processed: {len(full_results)} tweets in {batch_time:.2f}s. "
        ...
        while True:
            try:
                ...
                logger.error(f"Fatal error in streaming task: {e}",
                             exc_info=True)
                self.bq_queue.task_done(
                )  # Ensure queue tasks are marked as done even on error
                break

    async def process_tweets(
            self,
            fetch_size:
        int = 10000,  # How many rows to fetch from BigQuery at once
            total_limit: Optional[
                int] = None,  # Total rows to process (optional)
    ) -> None:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.info(
                        f"Retrieved {len(rows)} rows from BigQuery (offset {offset})"
                    )
            ...
            await self.bq_queue.join()  # Ensure all items are processed
            await self.bq_queue.put("DONE")  # Signal completion
            await self.streaming_task  # Wait for streaming task to finish
            ...
            avg_batch_time = (sum(self.batch_times) /
                              len(self.batch_times) if self.batch_times else 0)
            avg_queue_time = (sum(self.queue_times) /
                              len(self.queue_times) if self.queue_times else 0)
            ...
            logger.info(
                ...
                f"\n- Batch concurrency: {self.max_concurrent_batches}"
                f"\n- Batch size: {self.batch_size}"
                f"\n- Pipeline batch size: {self.pipeline_batch_size}")
