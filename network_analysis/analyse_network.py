...
# Define file path
file_name = "network_users_metrics_2025-02-10-all-users+mentions.csv"
...
def calculate_statistical_measures(df, metrics, normalize=True):
    ...
    for i, metric1 in enumerate(metrics):
        ...
        for metric2 in metrics[i +
                               1:]:  # Avoid self-comparisons and duplicates
            # Prepare data
            x = df[metric1].values
            y = df[metric2].values
...
def analyze_network_metrics(data_path, selected_metrics, normalize=True):
    ...
    stats_results = calculate_statistical_measures(df,
                                                   selected_metrics,
                                                   normalize=normalize)

    # Calculate lagged correlations
    lag_correlations = calculate_lagged_correlations(df,
                                                     selected_metrics,
                                                     normalize=normalize)
    ...
    plot_correlation_heatmap(df,
                             selected_metrics,
                             "Selected Metrics Correlations",
                             normalize=normalize)
    plot_metric_trends(df,
                       selected_metrics)  # Always normalized for comparison
    ...
    for metric1 in selected_metrics:
        for metric2 in selected_metrics:
            if metric1 < metric2:  # Avoid duplicates
                if metric1 in stats_results and metric2 in stats_results[
                        metric1]:
...
if __name__ == "__main__":
    # Run analysis
    stats_results, lag_correlations = analyze_network_metrics(output_path,
                                                              selected_metrics,
                                                              normalize=True)
    ...
    for metric1 in stats_results:
        for metric2 in stats_results[metric1]:
            ...
            print(f"  Strongest lag: {max_lag['lag']} months "
